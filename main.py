...
def main_loop(username):
    ...
    def about_func():
        mb.showinfo('About', 'This application was designed to have group chats. This has features like sending messages, showing all users in database, showing all the active users, working in real-time, responsive, managing account settings, login, logout, register. Hope you like the application.')
    ...
    def home_func():
        ...
        log_in_as.place(x=20,y=130)
        name.place(x=100,y=131)
    ...
    title_label = Label(win, text='UniChat',bg=bg,fg='white',font=('arial black',12,'bold'),bd=0)
    title_label.place(x=350,y=2)
    ...
    ## Group Chat
    chat_text = tks.ScrolledText(win, height=19, width=47,font=('Cambria',13),bg=sbg)
    chat_text.vbar.config(troughcolor = 'red', bg = 'blue')
    chat_text.place(x=10,y=35)
    chat_text.yview('end')
    ...
    ## User List
    hide_list = Button(win,fg='white',font=('arial black',10,'bold'),text='Hide User Lists',bg=bg, bd =0, width=15,command=hide_list_func).place(x=450,y=427)
    ...
    menu_bg = Label(win, text='',bg=bg,width=15,font=('arial black',15,'bold'),bd=0, relief='groove',height=16)
    log_bg = Label(win,bg=bg,width=30,height=9,relief='groove', bd= 0)
    login_img =PhotoImage(file='resources/login.png')
    log_img = Label(win,image=login_img,bg=bg)
    log_in_as = Label(win, text='Username:',bg=bg,font=('',13),fg='white')
    name = Label(win, text=username,bg=bg,font=('',11,'bold'),fg='white')
##    if len(username) > 25:
##        name['font'] = ('',9,'bold')
    ...
    all_user_listbox = Listbox(win, width=14, height=7,font=('Cambria',13),bg=sbg, bd=0)
    all_user_listbox.place(x=460,y=75)
    Label(win, bg=bg, width=14, text='Active Users', fg='White',font=('Cambria',13), bd=0).place(x=460,y=250)
    active_user_listbox = Listbox(win, width=14, height=6,font=('Cambria',13),bg=sbg, bd=0)
